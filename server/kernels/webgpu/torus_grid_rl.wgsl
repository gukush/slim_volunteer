...
fn weight_value(i: u32) -> f32 {
  return f32(atomicLoad(&weights_fixed[i])) / WEIGHT_SCALE;
}
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  for (var step = 0u; step < MAX_STEPS; step = step + 1u) {
    ...
    for (var a = 0u; a < ACTION_DIM; a = a + 1u) {
      var sum = 0.0;
      for (var f = 0u; f < FEATURE_DIM; f = f + 1u) {
        sum = sum + feats[f] * weight_value(f * ACTION_DIM + a);
      }
      logits[a] = sum;
    }
  ...
  atomicAdd(&stats_fixed[0], 1u);
  atomicAdd(&stats_fixed[1], hit_goal);
  atomicAdd(&stats_fixed[2], u32(round((total_reward + 512.0) * 1000.0)));
  atomicAdd(&stats_fixed[3], steps);
}
