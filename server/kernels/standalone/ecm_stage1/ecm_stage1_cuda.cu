...
// Now with in-app prime powers generation (highest p^k <= B1) and start confirmation.
...
static inline __host__ __device__ bool is_even(const U256 &a) {
...
static inline __host__ __device__ void addc(
  uint32_t a, uint32_t b, uint32_t cin,
...
static inline __host__ __device__ void subb(
  uint32_t a, uint32_t b, uint32_t bin,
  uint32_t &diff, uint32_t &bout
) {
  uint32_t d = a - b;
  uint32_t b1 = (a < b);
  uint32_t d2 = d - bin;
...
static inline __host__ __device__ U256 add_u256(const U256 &a, const U256 &b) {
  ...
  for (int i = 0; i < 8; i++) {
    uint32_t s, co;
    addc(a.limbs[i], b.limbs[i], c, s, co);
    r.limbs[i] = s;
    c = co;
  }
  return r;
...
static inline __host__ __device__ U256 sub_u256(const U256 &a, const U256 &b) {
  ...
  for (int i = 0; i < 8; i++) {
    uint32_t d, bo;
    subb(a.limbs[i], b.limbs[i], br, d, bo);
    r.limbs[i] = d;
    br = bo;
  }
  return r;
...
static inline __host__ __device__ void mul32x32_64(
...
static inline __host__ __device__ U256 mont_mul(
  const U256 &a, const U256 &b, const U256 &N, uint32_t n0inv32
) {
  uint32_t t[9];
  #pragma unroll
  for (int i = 0; i < 9; i++) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    for (int j = 0; j < 8; j++) {
      uint32_t plo, phi;
      mul32x32_64(a.limbs[i], b.limbs[j], plo, phi);

      uint32_t s1, c1;
      addc(t[j], plo, 0u, s1, c1);

      uint32_t s2, c2;
      addc(s1, carry, 0u, s2, c2);
    ...
    for (int j = 0; j < 8; j++) {
      uint32_t plo, phi;
      mul32x32_64(m, N.limbs[j], plo, phi);
...
static inline __host__ __device__ U256 mont_add(
  const U256 &a, const U256 &b, const U256 &N
) {
  return cond_sub_N(add_u256(a, b), N);
}

static inline __host__ __device__ U256 mont_sub(
  const U256 &a, const U256 &b, const U256 &N
) {
  if (cmp(a, b) >= 0) {
    return sub_u256(a, b);
...
static inline __host__ __device__ U256 mont_sqr(
  const U256 &a, const U256 &N, uint32_t n0inv32
) {
  return mont_mul(a, a, N, n0inv32);
}

static inline __host__ __device__ U256 to_mont(
  const U256 &a, const U256 &R2, const U256 &N, uint32_t n0inv32
) {
  return mont_mul(a, R2, N, n0inv32);
}

static inline __host__ __device__ U256 from_mont(
  const U256 &a, const U256 &N, uint32_t n0inv32
) {
...
// --------------------------- X-only ops ---------------------------
static inline __host__ __device__ PointXZ xDBL(
  const PointXZ &P, const U256 &A24, const U256 &N, uint32_t n0inv32
) {
...
static inline __host__ __device__ PointXZ xADD(
  const PointXZ &P, const PointXZ &Q, const PointXZ &Diff,
  const U256 &N, uint32_t n0inv32
) {
  U256 t1 = mont_add(P.X, P.Z, N);
  U256 t2 = mont_sub(P.X, P.Z, N);
...
static inline __host__ __device__ void cswap(
  PointXZ &a, PointXZ &b, uint32_t bit
) {
...
static inline __host__ __device__ PointXZ ladder(
  const PointXZ &P, uint32_t k, const U256 &A24, const U256 &N,
  uint32_t n0inv32, const U256 &mont_one
) {
...
static inline __host__ __device__ uint32_t lcg32(uint32_t &state) {
  uint32_t new_state = state * 1103515245u + 12345u;
  state = new_state;
  return new_state;
}

static inline __host__ __device__ U256 next_sigma(
  const U256 &/*N*/, uint32_t &lcg_state
) {
  U256 acc;
  #pragma unroll
  for (int i = 0; i < 8; i++) {
...
static inline __host__ __device__ InvResult mod_inverse(
  U256 a_in, const U256 &N
) {
  if (is_zero(a_in)) {
    return {false, set_zero()};
  }

  // try to reduce a minimally (up to 2 subs like WGSL)
  for (int k = 0; k < 2 && cmp(a_in, N) >= 0; k++) {
    a_in = sub_u256(a_in, N);
  }
  if (is_zero(a_in)) {
    return {false, set_zero()};
  }
...
static inline __host__ __device__ CurveResult generate_curve(
  const U256 &sigma, const U256 &N, const U256 &R2, uint32_t n0inv32
) {
  ...
  if (is_zero(sigma_std) || cmp(sigma_std, one) == 0 ||
      cmp(sigma_std, N_minus_1) == 0 || cmp(sigma_std, two) == 0 ||
      cmp(sigma_std, N_minus_2) == 0) {
...
__global__ void ecm_stage1_kernel(
  ...
  const uint32_t * __restrict__ pp, uint32_t pp_count,
  uint32_t pp_start, uint32_t pp_len,
  uint32_t seed_lo, uint32_t seed_hi, uint32_t base_curve, uint32_t flags,
  uint32_t * __restrict__ X_state, uint32_t * __restrict__ Z_state,
  uint32_t * __restrict__ A24_state,
  ...
  if (pp_start == 0u) {
    ...
    if (!curve_ok) {
      #pragma unroll
      for (int i = 0; i < 8; i++) {
        out_words[out_base + i] = 0u;
      }
      out_words[out_base + 8] = 3u; // bad curve
      return;
    }
  } else {
    // Resume state
  ...
  // Process prime powers window
  for (uint32_t i = pp_start; i < pp_end; ++i) {
    uint32_t e = pp[i];
    if (e > 1u) {
      R = ladder(R, e, A24m, N, n0inv32, mont_one);
    }
  }
  ...
  if (pp_end < pp_count) {
    ...
    #pragma unroll
    for (int i = 0; i < 8; i++) {
      out_words[out_base + i] = result.limbs[i];
    }
...
static std::string to_hex_le(const U256 &x) {
  ...
  for (int i = 7; i >= 0; i--) {
    oss << std::hex << std::setw(8) << std::setfill('0')
        << std::nouppercase << x.limbs[i];
  }
  return oss.str();
}

static uint32_t mont_n0inv32(uint32_t n0) {
  // Newton iterations modulo 2^32
  uint32_t inv = 1u;
  for (int i = 0; i < 5; i++) {
    inv *= (2u - n0 * inv);
  }
...
static std::vector<uint32_t> read_pp_file(const std::string &path) {
  ...
  if (!fin) {
    std::cerr << "Failed to open pp file: " << path << "\n";
    std::exit(1);
  }
  ...
  while (std::getline(fin, line)) {
    ...
    if (line.rfind("0x", 0) == 0 || line.rfind("0X", 0) == 0) {
      val = std::stoull(line, nullptr, 16);
    } else {
      val = std::stoull(line, nullptr, 10);
    }
...
static std::vector<uint32_t> generate_prime_powers(uint32_t B1) {
  ...
  powers.reserve(B1 / 10 + 10);

  for (uint32_t p = 2; p <= B1; ++p) {
    if (!sieve[p]) continue;
    if ((uint64_t)p * p <= B1) {
      for (uint32_t i = p * p; i <= B1; i += p) {
        sieve[i] = 0;
      }
    }
    uint64_t pk = p;
    while (pk * (uint64_t)p <= B1) {
      pk *= p;
    }
    powers.push_back((uint32_t)pk);
  }
  return powers; // ascending p, with p^k per prime
}
...
struct Args {
  ...
  uint32_t n_curves = 0;
  std::string pp_file;
  uint32_t B1 = 0; // optional bound to generate pp

  uint32_t seed_lo = 0;
  uint32_t seed_hi = 0;
  uint32_t base_curve = 0;
  uint32_t flags = 1;
...
static void usage_and_exit(const char *prog) {
  std::cerr
    << "Usage: " << prog
    << " --modulus 0x<64hex> --n-curves N (--pp-file file | --b1 B1)\n"
...
static Args parse_args(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    std::string k = argv[i];
    auto need = [&](int m) {
      if (i + m >= argc) usage_and_exit(argv[0]);
    };

    if (k == "--modulus") {
      need(1);
      haveN = true;
      if (!parse_hex_u256(argv[++i], a.N)) usage_and_exit(argv[0]);
    } else if (k == "--n-curves") {
      need(1);
      haveNC = true;
      a.n_curves = std::stoul(argv[++i]);
    } else if (k == "--pp-file") {
      need(1);
      a.pp_file = argv[++i];
    } else if (k == "--b1" || k == "--B1") {
      need(1);
      a.B1 = (uint32_t)std::stoul(argv[++i]);
    } else if (k == "--seed-lo") {
      need(1);
      a.seed_lo = std::stoul(argv[++i]);
    } else if (k == "--seed-hi") {
      need(1);
      a.seed_hi = std::stoul(argv[++i]);
    } else if (k == "--base-curve") {
      need(1);
      a.base_curve = std::stoul(argv[++i]);
    } else if (k == "--flags") {
      need(1);
      a.flags = std::stoul(argv[++i]);
    } else if (k == "--pp-start") {
      need(1);
      a.pp_start = std::stoul(argv[++i]);
    } else if (k == "--pp-len") {
      need(1);
      a.pp_len = std::stoul(argv[++i]);
    } else if (k == "--resume-in") {
      need(1);
      a.resume_in = argv[++i];
    } else if (k == "--resume-out") {
      need(1);
      a.resume_out = argv[++i];
    } else if (k == "--threads-per-block") {
      need(1);
      a.threads_per_block = std::stoi(argv[++i]);
    } else {
  ...
  if (a.pp_file.empty() && a.B1 == 0) {
    std::cerr << "Provide either --pp-file or --b1\n";
    std::exit(1);
  }
  if ((a.pp_len == 0u) && (a.pp_start != 0u)) {
    std::cerr << "If --pp-start is set, --pp-len must be > 0\n";
    std::exit(1);
  }
  return a;
}
...
int main(int argc, char **argv) {
  ...
  if (!args.pp_file.empty()) {
    pp = read_pp_file(args.pp_file);
  } else {
    pp = generate_prime_powers(args.B1);
  }
  if (pp.empty()) {
    std::cerr << "Prime powers are empty.\n";
    return 1;
  }
  ...
  if (pp_start > pp_count) {
    std::cerr << "pp_start beyond pp_count\n";
    return 1;
  }
  if (pp_start + pp_len > pp_count) {
    pp_len = pp_count - pp_start;
  }

  // Optional resume
  size_t n = args.n_curves;
  std::vector<uint32_t> X_h(n * 8, 0), Z_h(n * 8, 0), A24_h(n * 8, 0);
  std::vector<uint32_t> sigma_h(n, 0), ok_h(n, 0);
  ...
  if (!args.resume_in.empty()) {
    if (!read_state(args.resume_in)) {
      std::cerr << "Could not read resume state; starting fresh.\n";
    }
  }
  ...
  std::cout << "ECM Stage 1 (CUDA)\n"
            ...
            << "  pp_count: " << pp_count
            << "  window: [" << pp_start << ", " << (pp_start + pp_len) << ")\n"
  ...
  // Device buffers
  uint32_t *d_pp    = nullptr;
  uint32_t *dX      = nullptr;
  uint32_t *dZ      = nullptr;
  uint32_t *dA24    = nullptr;
  uint32_t *dSigma  = nullptr;
  uint32_t *dOk     = nullptr;
