// cublas_block_matmul_fp32.cu
// Build (works on Pascal+): 
//   nvcc -O3 -std=c++17 -arch=sm_70 -lcublas -o exe-block-matmul-cublas-fp32 cublas_block_matmul_fp32.cu
//
// Behavior:
// - Reads M, K, N (int32) then A[M*K], B[K*N] as float32 from stdin (row-major).
// - Uses cuBLAS SGEMM (pure FP32 on CUDA cores; TF32 OFF) to compute C = A @ B.
// - Writes C[M*N] as float32 to stdout (row-major).
//
// Notes:
// - Converts row-major host input to column-major device buffers for cuBLAS.
// - Forces math mode to CUBLAS_DEFAULT_MATH to avoid TF32 on Ampere+.
// - For extra safety you may set env NVIDIA_TF32_OVERRIDE=0 at runtime.
...
int main(){
    ...
    // Device buffers (column-major variants for cuBLAS)
    ...
    // Force pure FP32 math (no TF32)
    #ifdef CUBLAS_DEFAULT_MATH
    CUBLAS_CHECK(cublasSetMathMode(handle, CUBLAS_DEFAULT_MATH));
    #endif

    const float alpha = 1.0f, beta = 0.0f;
    // Column-major: C(MxN) = A(MxK) * B(KxN)
