// kernels/ecm_stage1_cuda_optimized.cu - Version 3 (resumable, 32-bit limbs) - FIXED
...
// - FIXED: Proper status codes and GCD computation
...
extern "C" {
    ...
    // Status codes matching WGSL
    static const uint32_t STATUS_NEEDS_MORE = 0u;
    static const uint32_t STATUS_NO_FACTOR = 1u;
    static const uint32_t STATUS_FACTOR_FOUND = 2u;
    static const uint32_t STATUS_BAD_CURVE = 3u;
    ...
    __device__ __forceinline__ bool is_even(const U256& a) {
    ...
    __device__ __forceinline__ void rshift1(U256& result, const U256& a) {
        uint32_t carry = 0u;
        for (int i = LIMBS - 1; i >= 0; i--) {
            uint32_t w = a.limbs[i];
            result.limbs[i] = (w >> 1u) | (carry << 31u);
            carry = w & 1u;
        }
    }
    ...
    __device__ __forceinline__ U256 gcd_binary_u256_oddN(const U256& a_in, const U256& N_odd) {
        U256 a, b;
        copy(a, a_in);
        copy(b, N_odd);
        ...
        // Remove factors of 2 from a (N is odd)
        while (is_even(a)) {
            rshift1(a, a);
        }
    ...
    __global__ void ecm_stage1_v3_optimized(uint32_t* io) {
        ...
        if (h.pp_start == 0u) {
            ...
            if (!curve_ok) {
                ...
                io[out_base + 8] = STATUS_BAD_CURVE;
                return;
            }
        } else {
        ...
        uint32_t pp_end = h.pp_start + h.pp_len;
        if (pp_end > h.pp_count) pp_end = h.pp_count;
        ...
        if (pp_end < h.pp_count) {
            ...
            // Write status 0 (needs more) to output - FIXED
            io[out_base + 8] = STATUS_NEEDS_MORE;
        } else {
            // Done - compute final result
            U256 result;
            uint32_t status = STATUS_NO_FACTOR; // default: no factor

            if ((h.flags & 1u) != 0u) {
                // Compute GCD(Z, N) to check for factor
                U256 Zstd;
                from_montgomery(Zstd, R.Z, N, n0inv32);
                U256 g = gcd_binary_u256_oddN(Zstd, N);
                copy(result, g);

                // Check if we found a nontrivial factor
                U256 one;
                set_one(one);
                if (!is_zero(g) && (cmp(g, N) < 0) && (cmp(g, one) > 0)) {
                    status = STATUS_FACTOR_FOUND; // factor found!
                } else {
                    status = STATUS_NO_FACTOR; // no factor
                }
            } else {
                // No GCD requested, just return Z
                from_montgomery(result, R.Z, N, n0inv32);
                status = STATUS_NO_FACTOR;
            }

            // Write final result
            for (uint32_t i = 0; i < 8; i++) {
                io[out_base + i] = result.limbs[i];
            }
            io[out_base + 8] = status; // FIXED: proper status code
        }
    }
