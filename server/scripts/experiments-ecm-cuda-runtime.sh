...
run_cfg () {
  ...
  for i in $(seq 1 "$REPEATS"); do
    echo "[$(date -Is)] ECM Stage1 | CUDA runtime | iter $i/$REPEATS | B1=$B1 curves=$CURVES chunk=$CHUNK"
    NODE_TLS_REJECT_UNAUTHORIZED=0 node ./scripts/test-ecm-stage1-native.mjs \
      --baseURL="$BASE_URL" \
      --N="$N" \
      --B1="$B1" \
      --total_curves="$CURVES" \
...
echo "All CUDA-runtime ECM Stage 1 runs completed."
