// scripts/test-exe-ecm-stage1.mjs — quick end-to-end check for exe strategy
...
// Target number N (hex string or decimal). Tiny demo default.
let N = argv.N;
if (!N) {
  const p = 65537n, q = 65539n; N = '0x' + (p * q).toString(16);
}
  ...
  return new Promise((resolve, reject) => {
    const req = lib.request(url, options, res => {
      ...
      res.on('end', () => {
        ...
        if (responseType === 'arraybuffer') return resolve(buf);
...
async function main() {
  ...
  const createPayload = {
    ...
    label,
    input: { N, total_curves, chunk_size, threads, B1, B2, seed, targetWindowBits },
    config: { framework: 'exe', backend, program, ...(binaryPath ? { binary: binaryPath } : null) }
  };

  const created = await api(baseURL, '/tasks', { method: 'POST', json: createPayload });
  if (!created || !created.id) { console.error('Failed to create task:', created); process.exit(1); }
  const taskId = created.id;
  console.log(`Task created: ${taskId}`);

  const startResult = await api(baseURL, `/tasks/${taskId}/start`, { method: 'POST' });
  if (!startResult || !startResult.ok) { console.error('Failed to start task:', startResult); process.exit(1); }
  ...
  while (true) {
    ...
    if (state === 'completed' || state === 'error' || state === 'canceled') { console.log(''); break; }
  }

  if (status.status !== 'completed') { console.error('Task did not complete successfully:', status); process.exit(2); }
  ...
  if (summary) {
    ...
    const preview = {}; for (const k of keys.slice(0, 10)) preview[k] = summary[k];
  ...
  console.log(`\nArtifacts written to: ${outDir} (should be changed but works)\n`);
}
