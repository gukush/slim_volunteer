...
export class TaskTimers{
  constructor(taskId, dir){
    ...
    this.roundTrip = new CSVWriter(path.join(dir, `chunk_timing_${taskId}.csv`),
      ['chunkId','replica','client_id','t_chunk_create','t_sent','t_client_recv','t_client_done','t_client_recv_abs','t_client_done_abs','t_server_recv','t_assembled','duration_ms','cpu_time_ms','gpu_time_ms']);
  }
  chunkRow(data){
    const {chunkId, replica, clientId, tCreate, tSent, tClientRecv, tClientDone, tClientRecvAbs, tClientDoneAbs, tServerRecv, tAssembled, cpuTimeMs, gpuTimeMs} = data;
    const duration = (tAssembled||tServerRecv||Date.now()) - (tCreate||this.start);
    this.roundTrip.append([chunkId, replica, clientId||'', tCreate, tSent, tClientRecv, tClientDone, tClientRecvAbs||'', tClientDoneAbs||'', tServerRecv, tAssembled, duration, cpuTimeMs || '', gpuTimeMs || '']);
  }
