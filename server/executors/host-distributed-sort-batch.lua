-- host-distributed-sort-batch.lua (CUDA-based distributed sorting with batch optimization)
-- Implements bitonic sort using CUDA kernels with reduced round trips
-- Processes multiple sort stages in batches to minimize CPU-GPU transfers

print("[lua/sort-batch] *** BATCH OPTIMIZED VERSION LOADED ***")
...
local MAX_BATCH_SIZE = 16   -- Maximum stages per batch (memory limit)

local DEFAULT_ENTRY_BY_FW = {
  cuda = "execute_task_batch",
  opencl = "execute_task",
  vulkan = "main"
}
...
local function get_entry_point(fw, payload)
  ...
  return DEFAULT_ENTRY_BY_FW[norm_fw(fw)] or "execute_task_batch"
end
...
----------------------------------------------------------------------
local function process_stages_batch(stages, data, padded_size, ascending, source, entry, grid, block, fw)
  ...
  for batch_start = 1, #stages, BATCH_SIZE do
    ...
    print(string.format("[lua/sort-batch] Processing batch %d-%d (%d stages)",
                       batch_start, batch_end, num_stages))

    -- Create stage data buffer (array of SortStage structures)
    local stage_data = ""
    for _, stage in ipairs(batch_stages) do
      -- Each SortStage is 8 bytes: 4 bytes stage + 4 bytes substage
    ...
    local task = {
      ...
      inputs      = {
        { b64 = b64encode(current_data) },      -- data buffer
        { b64 = b64encode(stage_data) }         -- stage parameters
      },
      outputSizes = { padded_size * 4 },       -- output data size
      grid        = grid,
      block       = block,
    }
    ...
    if type(result) ~= "table" or not result.ok then
      ...
      error("[lua/sort-batch] executor error in batch " .. batch_start .. "-" .. batch_end .. ": " .. tostring(err))
    end

    -- Get result data for next batch
    ...
    print(string.format("[lua/sort-batch] Batch %d-%d completed in %.1fms (%.1fms/stage)",
...
function compile_and_run(chunk)
  print("[lua/sort-batch] compile_and_run - CUDA bitonic sort (batch optimized)")
  print("[lua/sort-batch] *** DEBUG: Starting batch-optimized compile_and_run ***")
  ...
  if fw ~= "cuda" then
    print("[lua/sort-batch] WARNING: requested fw="..tostring(fw)..", but this host is tuned for CUDA. Proceeding anyway.")
  end
  ...
  print(string.format("[lua/sort-batch] Processing %d integers (padded to %d), ascending=%s",
  ...
  print(string.format("[lua/sort-batch] Grid: %dx%dx%d, Block: %dx%dx%d",
                      grid[1], grid[2], grid[3], block[1], block[2], block[3]))

  -- Generate all sort stages
  local stages = generate_sort_stages(padded_size)
  print(string.format("[lua/sort-batch] Generated %d sort stages, processing in batches of %d",
                     #stages, BATCH_SIZE))

  -- Process stages in batches
  local final_data, total_gpu_time, stage_count = process_stages_batch(
    stages, padded_data, padded_size, ascending, source, entry, grid, block, fw
  )
  ...
  print(string.format("[lua/sort-batch] Bitonic sort complete: %d stages in %d batches, %.1fms total GPU time",
                      stage_count, math.ceil(#stages / BATCH_SIZE), total_gpu_time))

  return final_result_obj
end

print("[lua/sort-batch] CUDA bitonic sort host (batch optimized) loaded")
