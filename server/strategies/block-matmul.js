...
export const id = 'block-matmul';
export const name = 'Block Matrix Multiply (WebGPU)';
export const framework = 'webgpu';

export function getClientExecutorInfo(config){
  return {
    framework,
    path: 'executors/webgpu-block-matmul.client.js',
    kernels: ['kernels/block_matmul.wgsl'],
...
export function buildChunker({ taskId, taskDir, K, config, inputFiles }){
  ...
  return {
    async *stream(){
      ...
      fs.closeSync(fdA);
      fs.closeSync(fdB);
      logger.info('Block-matmul chunker done');
    }
  };
}
...
export function buildAssembler({ taskId, taskDir, config }){
  ...
  return {
    ...
    finalize(){
      const buf = Buffer.from(C.buffer);
